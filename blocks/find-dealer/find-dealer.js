const SAMPLE_DATA = [
  ...
  {
    name: 'Hyundai Northgate',
    address: '456 Aurora Ave N, Seattle, WA 98103',
    phone: '(206) 555-0456',
...
function renderDealers(block, dealers, bridge) {
  ...
  if (!dealers || dealers.length === 0) {
    ...
    emptyCard.className = 'dealer-search-card';
    ...
    searchBtn.textContent = 'Find a Dealer';
    if (bridge) {
      searchBtn.addEventListener('click', () => {
        const zip = input.value.trim();
        if (zip) {
          bridge.sendMessage(`Find a Hyundai dealer near ${zip}`);
        }
      });
    }
    ...
    const dealersRow = document.createElement('div');
    dealersRow.className = 'dealers-row';
    ...
    dealers.slice(0, 2).forEach(dealer => {
      ...
      dealersRow.appendChild(card);
    });

    container.appendChild(dealersRow);
  }
