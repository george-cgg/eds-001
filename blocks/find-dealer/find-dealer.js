...
const SAMPLE_DATA = [
  {
    name: "Auto Cobalcescu",
    address: "Bd. Cobalcescu 133, București",
    phone: "+40 21 310 3030",
    services: ["Sales", "Service", "Parts"]
  },
  {
    name: "Dacia Pipera",
    address: "Șoseaua Pipera 40, București",
    phone: "+40 21 232 5050",
...
function renderSearchForm(block, bridge) {
  ...
  heading.textContent = 'Găsește un dealer Dacia';
  card.appendChild(heading);

  const input = document.createElement('input');
  input.type = 'text';
  input.className = 'search-input';
  input.placeholder = 'Introdu orașul sau regiunea...';
  card.appendChild(input);

  const button = document.createElement('button');
  button.className = 'search-button';
  button.textContent = 'Caută dealer';
  if (bridge) {
...
function renderDealers(block, dealers, bridge) {
  ...
  displayDealers.forEach((dealer) => {
    ...
    iconCircle.className = 'dealer-icon';
