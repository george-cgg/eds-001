...
// State management - will be persisted in widget state
...
export default async function decorate(block, llmContext) {
  ...
  llmContext.on('displayMode', (displayMode) => {
    ...
    // Request fullscreen when form is active (step 2)
    if (displayMode === 'inline' && currentStep === 2) {
      // Optionally request fullscreen for better form experience
